...
const DeactivateMember = ({ setIsOpenInfoUser, isDeactivate, setIsDeactivate, infoMember, setCallback, callBack }) => {
  const { tokenInformation } = useGlobalContext();
  const handleEnabledMember = async () => {
    try {
      if (infoMember?.enabled === 0) {
        await account.changeStatusMember(infoMember.userId, 1);
        const dataAcivity = {
          userId: infoMember.userId,
          authorId: tokenInformation?.sub,
          command: 'CMD_ACTIVATED_USER',
        };
        await account.createActivity(dataAcivity);
      } else {
        await account.changeStatusMember(infoMember.userId, 0);
        const dataAcivity = {
          userId: infoMember.userId,
          authorId: tokenInformation?.sub,
          command: 'CMD_DEACTIVATED_USER',
        };
        await account.createActivity(dataAcivity);
      }
      ...
      toast.custom((t) => (
        <ToastCustom
          status={true}
          title={` ${infoMember?.enabled === 0 ? 'Inactive successfully' : 'Activate successfully'} `}
  ...
  return (
    <div>
      <ModalComponent isOpen={isDeactivate} setIsOpen={setIsDeactivate}>
        <div className="w-[480px] p-6 flex flex-col gap-3">
          ...
          <div>
            <p className="text-text-200 font-semibold text-lg">
              {infoMember?.enabled === 0 ? 'Activate Member' : 'Deactivate Member'}{' '}
            </p>
            <p className="text-sm font-normal text-text-100">
              {infoMember?.enabled === 0
                ? 'Are you sure you want to activate this member? Once activated, the member will regain access to all assigned permissions and resources in the system.'
                : ' Are you sure you want to deactivate this member? This member can be recovered if needed.'}{' '}
            </p>
          </div>
          <div className="flex gap-5 justify-end items-center">
            <div className="cursor-pointer" onClick={() => setIsDeactivate(false)}>
              <p className="text-sm">Cancel</p>
            </div>
            <div className="cursor-pointer">
              <p
                className={` ${infoMember?.enabled === 0 ? 'bg-[#0C66E4]' : 'bg-[#C9372C]'}  rounded text-sm text-white p-2`}
                onClick={() => handleEnabledMember()}
              >
                {infoMember?.enabled === 0 ? 'Activate Member' : 'Deactivate'}
              </p>
            </div>
          </div>
        </div>
