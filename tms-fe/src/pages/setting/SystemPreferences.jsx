...
import { GET_NOTIFY_SETTING, SET_NOTI_SETTING } from 'apis/notification/notification';
...
const SystemPreferences = () => {
    ...
    const { data: getDataSettingNoti } = useQuery(GET_NOTIFY_SETTING, {
        client: clientRepo,
        variables: { testPlanId: testPlanId, userId: userInfo?.userID },
        fetchPolicy: 'cache-and-network'
    });

    const initialCheckedValues = getDataSettingNoti?.getNotifySetting?.split(', ');
    ...
    const [setNotiSetting] = useMutation(SET_NOTI_SETTING, { client: clientRepo });
    const handleSubmitSetting = async () => {
        try {
            await setNotiSetting({
                variables: {
                    testPlanId: testPlanId,
                    userId: String(userInfo.userID),
                    notifySetting: checkedValues.join(', ')
                }
            });
        } catch (error) {
            toast.error('Error');
